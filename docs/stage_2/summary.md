...
**Tests added**: 22 (approval suite) → total unit test count: 56
...
|--------|---------------|
| `store.py` | In-process `PendingStore` singleton with dual dict lookup (`session_id → request_id`, `request_id → ApprovalRequest`); `add`, `get_pending_for_session`, `get_by_request_id`, `record_decision`, `is_expired`, `clear_session`. |
...
- **`pending_check_node`** — fires first on every turn; checks store by `session_id`; delivers decision (approved/rejected) or expiry message if applicable; leaves state untouched if no decision yet (allowing free interaction while pending).
- **`guard_rails_node`** — bypass extended to cover all approval-related statuses (`pending_approval`, `approved`, `rejected`, `expired`), preventing false-positive PII hits on reservation confirmations.
...
| `test_approval_nodes.py` | 5 | approved, rejected+reason, expired, still-pending, no-pending |
...
**PII bypass scoping** — Extending the guard-rails bypass from `status == "submitted"` to the full approval status set was caught early and handled cleanly, avoiding a class of false-positive failures that would have been confusing in demo.
...
| Unit tests (approval) | 22 |
| Unit tests (total) | 56 |
| New source files | 5 (`models`, `store`, `notifier`, `service`, `__init__`) |
