...
## Stages 1–4 · RAG Foundation · Reservation · HITL Approval · MCP Storage

---

**Overview · Architecture · RAG · Guard Rails · Reservation · HITL · MCP Storage · Evaluation**
...
- **Stage 4**: MCP server writes approved reservations to audit file
...
<div class="note">Delivery: 5 user stories · 71 unit tests · CI on GitHub Actions · Docker Compose full-stack</div>
...
**→** All 71 unit tests run without Milvus installed — CI stays fast.
...
| `test_approval_api.py` | 7 | 204, 401, 404, 409 + storage mock |
| `test_approval_nodes.py` | 6 | approved, rejected, expired, fresh-state |
| `test_storage_writer.py` | 6 | file create, format, append, sanitization |
| `test_storage_server.py` | 3 | tool handler direct-call, validation |
| `test_storage_client.py` | 3 | args passthrough, is_error → RuntimeError |

<div class="bar green">✅ pytest tests/unit/ → **71 passed** (< 2 s, no external services)</div>
...
- API tests: `ReservationStorageClient` autouse mock prevents subprocess spawn
...
<!-- ══════════════════════════════════════════════════════════════════
     SLIDE 14 — Stage 4 MCP Architecture
     ══════════════════════════════════════════════════════════════════ -->
<!-- _class: mcp -->
...
<!-- ══════════════════════════════════════════════════════════════════
     SLIDE 15 — Stage 4 MCP Demo
     ══════════════════════════════════════════════════════════════════ -->
<!-- _class: mcp -->

# Stage 4 — End-to-End Walkthrough
...
<div class="note">Prerequisite: package installed in container (`pip install -e .` in Dockerfile) so subprocess can import `chatbot` · 14 new unit tests across 4 modules · Full walkthrough: specs/003-mcp-reservation-storage/quickstart.md</div>
...
- Integration test: real MCP subprocess + `tmp_path` file assertion
...
- Persistent MCP server process (one per API worker) for lower latency
...
- Storage file rotation / archival policy
...
Stage 1 ✅ RAG Foundation &nbsp;·&nbsp; Stage 3 ✅ HITL Approval &nbsp;·&nbsp; Stage 4 ✅ MCP Storage &nbsp;·&nbsp; Stage 5: production hardening
